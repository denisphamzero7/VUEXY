...
export function defineAbilityFor(user) {
  ...
  // Process role permissions (from user.role.permissions)
  ...
  // Dynamic permission rules based on user context
...
function applyDynamicRules(can, cannot, user) {
  ...
  case 'manager':
    ...
    can('read', 'Dashboard')
    can('read', 'User') // Explicit read permission
    break

  case 'user':
    console.log('Applying user permissions - read only')
    can('read', 'Dashboard')
    can('read', 'User') // Can view user list and details
